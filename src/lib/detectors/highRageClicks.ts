...
export async function detectHighRageClicks(opts: {
  ...
  for (const r of (data ?? []) as Row[]) {
    ...
    const pct = Number(r.rage_click_pct);
    if (sessions < minSessions || pct < pctThreshold) continue;
    const severity = pct >= 15 ? 'high' : pct >= 8 ? 'medium' : 'low';
    rows.push({
      workspace_id: opts.workspaceId,
      detector: 'high_rage_clicks',
      category: 'ux',
      severity,
      polarity: 'issue',
      title: `${pct.toFixed(1)}% of sessions rage-clicked on ${r.page_path}`,
      evidence: { page_path: r.page_path, rage_click_pct: pct, sessions, window_days: windowDays },
      sources: ['Clarity'],
      dedupe_key: `high_rage_clicks:${r.page_path}`,
    });
  }
  return writeInsights({ workspaceId: opts.workspaceId, detector: 'high_rage_clicks', rows });
}
