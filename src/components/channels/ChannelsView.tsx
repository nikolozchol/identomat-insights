...
export type ChannelRow = { channel: string; sessions: number; conversions: number; convRate: number; share: number };
...
export function ChannelsView({ channels, totals, windowLabel }: {
  channels: ChannelRow[];
  ...
  const sorted = [...channels].sort((a, b) => (dir === 'desc' ? b[sortKey] - a[sortKey] : a[sortKey] - b[sortKey]));

  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-5 px-6 py-6">
      <div className="flex items-baseline gap-2">
        <h1 className="text-[18px] font-medium tracking-[-0.01em] text-fg">Channels</h1>
